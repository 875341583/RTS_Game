...
public partial class Unit : CharacterBody2D, IUnitEntity
{
    ...
    private static void EnsureIsoSprites()
    {
        ...
        GameLog.Debug($"[R3] Isometric sprites loaded: {_isoSprites.Count} unit types");
    }
    ...
    public void InitAsType(UnitType type)
    {
        ...
        if (s.IsHero)
        {
            ...
            Health = MaxHealth;
            GameLog.Debug($"[E6b] Hero skill: {_heroSkill}");
        }
    }
    ...
    public void EmbarkPassenger(Unit passenger)
    {
        ...
        GameLog.Debug($"[IFV] {passenger.UnitName} embarked into {UnitName} (occupants {Passengers.Count}/{MaxPassengers})");
    ...
    public void DisembarkAll()
    {
        ...
        GameLog.Debug($"[IFV] {Passengers.Count} passengers disembarked from {UnitName}");
        Passengers.Clear();
    ...
    private void ApplyMergeEffect(UnitType passengerType)
    {
        ...
        switch (passengerType)
        {
            case UnitType.Sapper:
                ...
                MaxHealth = 200f;
                Health = 200f;
                GameLog.Debug("[IFV] Merge: Sapper Vehicle (repair + terrain modification)");
                break;
            case UnitType.ChiefEngineer:
                ...
                MaxHealth = 250f;
                Health = 250f;
                GameLog.Debug("[IFV] Merge: Advanced Sapper Vehicle (advanced modification)");
                break;
            case UnitType.Infantry:
                ...
                AggroRange = 250f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Armed Jeep (light machine gun)");
                break;
            case UnitType.Grenadier:
                ...
                AggroRange = 280f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Assault Gun (AOE firepower)");
                break;
            case UnitType.Sniper:
                ...
                AggroRange = 400f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Sniper Tank (long-range precision)");
                break;
            case UnitType.FlameInfantry:
                ...
                AggroRange = 150f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Flame Tank (close-range high DPS)");
                break;
            // E6b：特殊单位IFV合体
            case UnitType.Hero:
                // 英雄→英雄战车：超强火力
                ...
                AggroRange = 300f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Hero Tank (super firepower)");
                break;
            case UnitType.Spy:
                // 间谍→间谍车：渗透战车
                ...
                Health = 180f;
                MoveSpeed = 280f;
                GameLog.Debug("[IFV] Merge: Spy Vehicle (high-speed infiltration)");
                break;
            case UnitType.Thief:
                // 窃贼→劫掠车：偷钱战车
                ...
                AggroRange = 180f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Raider (money-stealing vehicle)");
                break;
            default:
                // 其他步兵→轻型武装车
                ...
                AggroRange = 220f;
                AutoDefend = true;
                GameLog.Debug("[IFV] Merge: Light Armed Vehicle");
                break;
        }
    ...
    private void ProcessSpyInfiltrate(float dt)
    {
        ...
        if (_spyMission.HasValue && _spyTargetBuilding != null)
        {
            ...
            {
                GameLog.Debug("[G7] Spy mission cancelled: target building no longer exists");
                _spyMission = null;
            ...
            {
                // 太远了，取消任务
                GameLog.Debug($"[G7] Spy mission cancelled: too far from target ({(int)dist}px)");
                _spyMission = null;
                _spyTargetBuilding = null;
                _spyMissionTimer = 0f;
                return;
            ...
            if (_spyMissionTimer <= 0f)
            {
                ...
                if (success)
                {
                    GameLog.Debug($"[G7] Spy mission succeeded: {SpyMission.MissionName(missionType)} → {target.BuildingName} (Team {target.TeamId})");
                    // 通知Main执行任务效果
                    if (GetParent()?.GetParent() is Main mainNode)
                    {
                ...
                {
                    GameLog.Error($"[G7] Spy mission failed: {SpyMission.MissionName(missionType)} — spy killed!");
                    // 间谍死亡
                ...
                && b.GlobalPosition.DistanceTo(GlobalPosition) < 60f)
            {
                ...
                if (_spyInfiltrateTimer >= 4f)
                {
                    ...
                    if (main is Main mainNode)
                    {
                        ...
                        GameLog.Debug($"[E6b] Spy stole ${stolen} (Team {b.TeamId} → Team {TeamId})");
                    }
                }
                break;
            }
            ...
            foreach (var c in bnode.GetChildren())
            {
                    ...
                    && b.GlobalPosition.DistanceTo(GlobalPosition) < 60f)
                {
                    ...
                    if (stolen > 0)
                    {
                        ...
                        GameLog.Debug($"[E6b] Thief stole ${stolen} (Team {b.TeamId} → Team {TeamId})");
                    }
                    return;
                }
            }
            ...
            foreach (var c in unitsNode.GetChildren())
            {
                    ...
                    && u is Harvester && u.GlobalPosition.DistanceTo(GlobalPosition) < 60f)
                {
                    ...
                    if (stolen > 0)
                    {
                        ...
                        GameLog.Debug($"[E6b] Thief stole from harvester ${stolen} (Team {u.TeamId} → Team {TeamId})");
                    }
                    return;
                }
            }
    ...
    public void CommandSpyMission(Building target, SpyMission.MissionType mission)
    {
        ...
        // P0-1: 清除旧路径
        ClearPath();
        GameLog.Debug($"[G7] Spy starting mission: {SpyMission.MissionName(mission)} → {target.BuildingName}");
    }
    ...
    private void ProcessTerrainModification(float dt)
    {
        ...
        if (!_isConstructing)
        {
            ...
            if (GetParent()?.GetParent() is Main mainNode)
            {
                ...
                {
                    // 资金不足
                    GameLog.Error($"[TerrainMod] {UnitName} (Team {TeamId}) insufficient funds $_terrainModCost, unable to construct");
                    _terrainModType = TerrainModType.None;
                    _isConstructing = false;
                    return;
                }
            ...
            GameLog.Debug($"[TerrainMod] {UnitName} (Team {TeamId}) started {_terrainModType} construction, cost ${_terrainModCost}, duration {_terrainModDuration:F1}s");
        }
    ...
    private void ExecuteTerrainMod()
    {
        ...
        switch (_terrainModType)
        {
            case TerrainModType.Flatten:
                ...
                GameLog.Debug($"[TerrainMod] Mountain flattened ({gx},{gy})");
                break;
            ...
            case TerrainModType.Tunnel:
                ...
                cell.HasTunnel = true;
                terrain.SetCell(gx, gy, cell);
                GameLog.Debug($"[TerrainMod] Tunnel opened ({gx},{gy})");
                break;
            ...
            case TerrainModType.Bridge:
                ...
                cell.HasBridge = true;
                terrain.SetCell(gx, gy, cell);
                GameLog.Debug($"[TerrainMod] Bridge built ({gx},{gy})");
                break;
            ...
            case TerrainModType.UnderseaTunnel:
                ...
                terrain.SetCell(gx, gy, cell);
                GameLog.Debug($"[TerrainMod] Undersea tunnel completed ({gx},{gy})");
                break;
        }
    ...
    private void CheckLevelUp()
    {
        while (_level < 4 && _experience >= LevelThresholds[_level])
        {
            ...
            if (ability == UnitAbility.ReconVision)
                AggroRange *= 1.5f;
            // 掠夺：需要Main配合（在Die中回调）
            GameLog.Debug($"[E11] {UnitName} upgraded to Lv{_level}! Gained ability: {AbilityName(ability)}");
        }
    }
